...
export default function LoginClient() {
  ...
  const [error, setError] = useState(searchParams.get('error') === 'auth' ? 'Đăng nhập thất bại, vui lòng thử lại.' : '')
      ...
      <div className="bg-white rounded-2xl shadow-sm p-8 w-full max-w-md">
        ...
        <div className="space-y-3 mb-5">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-[#1877f2] hover:bg-[#166fe5] rounded-xl py-2.5 text-sm font-medium text-white transition-colors"
          >
        ...
        <div className="relative mb-5">
          ...
          <div className="relative flex justify-center">
            <span className="bg-white px-3 text-xs text-gray-400">hoặc dùng email</span>
          </div>
        </div>
        ...
        <form onSubmit={handleEmail} className="space-y-4">
          ...
          {message && (
            <p className="text-green-600 text-sm bg-green-50 border border-green-200 rounded-xl px-4 py-2.5">{message}</p>
          )}
